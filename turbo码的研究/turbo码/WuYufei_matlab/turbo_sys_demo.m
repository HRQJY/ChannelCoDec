...
% Clear bit error counter and frame error counter
   ...
   while nferr(nEN, niter)<ferrlim
      nframe = nframe + 1;    
      x = round(rand(1, L_total-m));    % 发送的信息比特info. bits
      [temp, alpha] = sort(rand(1,L_total));        % 随机交织映射random interleaver mapping
      en_output = encoderm( x, g, alpha, puncture ) ; % 进行turbo编码 (+1/-1)
