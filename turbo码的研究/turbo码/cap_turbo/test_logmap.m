    ...
    for nEN=1:length(snr)
        ...
        if nEN==1 | ber(iter(ii),nEN-1)>9.0e-6
            while nferr(iter(ii),nEN)<ferrlim        %nferr:当前迭代次数、snr点的错误帧数
                ...
                [hard_out,soft_out]=deturbo_logmap(soft_in,alphain,iter(ii)); %译码
        ...
        %save cap算法06_准Th_log_map.mat snr ber;
    end
end
semilogy(snr,ber(1,:),snr,ber(2,:),snr,ber(3,:));
xlabel('SNR(dB)');
ylabel('Bit Error Rate');
title('3GPP标准 准Threshold-log-map译码算法 译码性能图,1024交织长度，WYF噪声加法');
