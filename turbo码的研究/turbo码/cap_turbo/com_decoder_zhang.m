...
%---初始化&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
Infty = -128;
d(1:8,1:2,1:in_length)=zeros(8,2,in_length);    %分支量度，8种可能状态，输入为0或者1
                                %D(S,i,k)
a(1:8,1:in_length)=Infty*ones(8,in_length);     %前向分支量度，A(S,k)
a(1,1)=0;                       %寄存器状态由全零开始
b(1:8,1:in_length+1)=Infty*ones(8,in_length+1);     %后向分支量度，B(S,k)
b(1,in_length)=0;                       %寄存器状态由全零结束

for k=1:in_length

    d(1,2,k)=(x(k)+y(k)+pii*app(k))/2;
    d(2,2,k)=d(1,2,k);
    d(7,2,k)=d(1,2,k);
    d(8,2,k)=d(1,2,k);
    
 %   d(1,1,k)=-d(1,2,k); %(x(k)+y(k)+pii*app(k))
 %   d(2,1,k)=d(1,1,k);
 %   d(7,1,k)=d(1,1,k);
 %   d(8,1,k)=d(1,1,k);
    
%    d(3,2,k)=(x(k)-y(k)+pii*app(k))/2;
    d(3,2,k)=(x(k)+pii*app(k))/2;
%    d(3,2,k)=x(k)+pii*app(k);
    d(4,2,k)=d(3,2,k);
    d(5,2,k)=d(3,2,k);
    d(6,2,k)=d(3,2,k);
    
%    d(3,1,k)=-d(3,2,k); %-x(k)+y(k)-pii*app(k)
%    d(3,1,k)=y(k)-pii*app(k);
   d(3,1,k)=y(k)/2;
%    d(3,1,k)=y(k);
    d(4,1,k)=d(3,1,k);
    d(5,1,k)=d(3,1,k);
...
soft_out=l;
%ex_info=0.625*soft_out+0.375*app;
ex_info=(soft_out-app-x)*S;